...
def train_from_config(config_path: str) -> None:
    ...
    # --- Plot: Random Forest curve (for single-feature datasets) ---
    if len(cfg["data"]["features"]) == 1:
        feature_name = cfg["data"]["features"][0]
        x_min = float(X[feature_name].min())
        x_max = float(X[feature_name].max())

        X_grid = pd.DataFrame({feature_name: np.linspace(x_min, x_max, 300)})
        ...
        plt.title("Random Forest Regression: Level vs Salary")
        ...
        print("Plot skipped: only implemented for single-feature regression.")


if __name__ == "__main__":
    train_from_config("configs/regression/random_forest.yaml")
